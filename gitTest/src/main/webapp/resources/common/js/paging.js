...
paging.ajaxSubmit = function( url, dataNode, callbackFunc, asyncType, dataType ) {
	...
	$.ajax({
		beforeSend: function( xmlHttpRequest ){					// Interceptor에서 ajax를 판독하기 위함
			...
			//load image fade in 2018_04_20 추가
			
			var loadingHtml = "<div id='loading' style='z-index:1005;position:absolute;top:50%;left:50%;text-align:center;'>";
			loadingHtml +="<div class='loading_box'><img src='/spring/resources/common/design/img/load_root.gif'/></div><div>";
			$('body > div.wrap').fadeTo("fast",0.4).append(loadingHtml);
		},
...
//		contentType : "application/json; charset=UTF-8",		// 넣으면 데이터가 안넘어감..
		...
		,complete : function(){
			...
			$("body > div.wrap").fadeTo("slow",1).find("#loading").remove();
		}
	});
